...
# Debug variant of insights-per-project.sh — same loop, but invokes the VERBOSE
# expect script (run-insights-tui-debug.exp) so you can watch each project's
# TUI run live and diagnose why report.html isn't updating.
#
# For each parked project in ~/backup/projects/:
#   - move it back into ~/.claude/projects/
#   - clear usage-data so /insights rebuilds clean
#   - run /insights via the DEBUG expect driver (full TUI output to terminal)
...
#                    can re-run run-insights-tui-debug.exp against it manually.
...
for d in "$BACKUP_PROJECTS"/*/; do
  ...
  expect_rc=0
  ~/insights-badger/run-insights-tui-debug.exp || expect_rc=$?
  ...
  if [[ "$failed" -eq 1 ]] && [[ "$STOP_ON_FAIL" == "1" ]]; then
    ...
    echo "Re-run manually with: ~/insights-badger/run-insights-tui-debug.exp"
